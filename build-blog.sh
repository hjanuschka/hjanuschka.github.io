...
# 
# Posts are auto-discovered from *.md files with front matter:
#   ---
...
# NOTE: Not using set -e because grep/description extraction can fail on some posts
# set -e
...
const mode = process.argv[3]; // 'metadata' or 'content'

if (!mdFile) {
  console.error('Usage: node build-markdown.js <markdown-file> [metadata|content]');
  process.exit(1);
}
...
generate_html() {
    ...
    local content=$(node build-markdown.js "$md_file" content)
    
    # Extract first paragraph for description (remove HTML tags)
    local description=$(echo "$content" | grep -m1 "<p><em>" | sed 's/<[^>]*>//g' | sed 's/&[^;]*;//g' | cut -c1-160 || true)
...
# Files to exclude from blog processing (not blog posts)
EXCLUDE_PATTERNS="README|OSAKA_JADE|osaka-jade-theme-report"

# Counter for processed files
PROCESSED=0
SKIPPED=0

echo -e "${BLUE}Scanning for blog posts...${NC}"
...
for md_file in *.md; do
    ...
    # Skip excluded files
    if echo "$md_file" | grep -qE "$EXCLUDE_PATTERNS"; then
        echo -e "${YELLOW}⏭ Skipping (excluded): $md_file${NC}"
        ((SKIPPED++))
        continue
    fi
    ...
    if ! head -1 "$md_file" | grep -q "^---$"; then
        echo -e "${YELLOW}⚠ Skipping (no front matter): $md_file${NC}"
        echo -e "${YELLOW}  Add front matter to include this file:${NC}"
        echo -e "${YELLOW}  ---${NC}"
        echo -e "${YELLOW}  title: \"Your Title\"${NC}"
        echo -e "${YELLOW}  category: \"Category\"${NC}"
        echo -e "${YELLOW}  tech: \"Tech Stack\"${NC}"
        echo -e "${YELLOW}  ---${NC}"
        ((SKIPPED++))
        continue
    fi
    ...
    metadata=$(node build-markdown.js "$md_file" metadata 2>/dev/null)
    
    if [ -z "$metadata" ] || [ "$metadata" = "{}" ]; then
        echo -e "${YELLOW}⚠ Skipping (empty front matter): $md_file${NC}"
        ((SKIPPED++))
        continue
    fi
    
    # Parse metadata
    title=$(echo "$metadata" | node -pe "JSON.parse(require('fs').readFileSync('/dev/stdin').toString()).title || ''")
    category=$(echo "$metadata" | node -pe "JSON.parse(require('fs').readFileSync('/dev/stdin').toString()).category || 'Chromium'")
    tech=$(echo "$metadata" | node -pe "JSON.parse(require('fs').readFileSync('/dev/stdin').toString()).tech || 'C++'")
    
    if [ -z "$title" ]; then
        echo -e "${YELLOW}⚠ Skipping (no title in front matter): $md_file${NC}"
        ((SKIPPED++))
        continue
    fi
    ...
    # Generate the HTML
    generate_html "$md_file" "$output_file" "$title" "$category" "$tech"
    ((PROCESSED++))
done
...
echo -e "${BLUE}  Processed: $PROCESSED files${NC}"
