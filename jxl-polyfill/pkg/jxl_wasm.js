 ...
 * @param {Uint8Array} jxl_data
 * @returns {Uint8Array}
 */
export function decode_jxl_to_png(jxl_data) {
    const ptr0 = passArray8ToWasm0(jxl_data, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.decode_jxl_to_png(ptr0, len0);
    if (ret[3]) {
 ...
 * @returns {ImageInfo}
 */
export function get_jxl_info(jxl_data) {
    const ptr0 = passArray8ToWasm0(jxl_data, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.get_jxl_info(ptr0, len0);
    if (ret[2]) {
...
export class ImageInfo {
     ...
     */
    get num_frames() {
        const ret = wasm.imageinfo_num_frames(this.__wbg_ptr);
        return ret >>> 0;
    }
    /**
     * @returns {number}
     ...
     */
    get has_alpha() {
        const ret = wasm.imageinfo_has_alpha(this.__wbg_ptr);
        return ret !== 0;
    }
}
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('jxl_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
